...
def create_random_products(random_factory, amount_of_products):
    random_products = [dict(length=250, width=250, weight=1), dict(length=600, width=600, weight=4),
                       dict(length=1100, width=600, weight=6)]
...
def create_random_objects(random_factory, amount_of_machines, amount_of_warehouses, amount_of_agvs, STATIC):
    ...
    random_x_positions_for_objects = [x + 1 for x in range(random_factory.length - 1) if x % 4 == 0]
    random_y_positions_for_objects = [y + 1 for y in range(random_factory.width - 1) if y % 4 == 0]
    ...
    random_x_positions_for_agvs = [x + 1 for x in range(random_factory.length - 1) if x % 4 == 0]
    random_y_positions_for_agvs = [y + 1 for y in range(random_factory.width - 1) if y % 4 == 0]
    ...
    print(f"Random_Object_Positions = {random_object_positions}")
    print(f"Random_AGV_Positions = {random_agv_positions}")
    for i in range(amount_of_machines):
        random_factory.machines.append(Machine())
        random_factory.machines[i].name = f'M{i}'
        ...
        random_factory.machines[i].length = 3
        random_factory.machines[i].width = 3
        random_factory.machines[i].pos_input = [random_factory.machines[i].pos_x + 1, random_factory.machines[i].pos_y]
        random_factory.machines[i].pos_output = [random_factory.machines[i].pos_x + 1,
                                                 random_factory.machines[i].pos_y + 2]
    ...
    for i in range(amount_of_warehouses):
        ...
        random_factory.warehouses[i].name = f'W{i}'
        ...
        random_factory.warehouses[i].length = 3
        random_factory.warehouses[i].width = 3
        random_factory.warehouses[i].pos_input = [random_factory.warehouses[i].pos_x + 1,
                                                  random_factory.warehouses[i].pos_y]
        random_factory.warehouses[i].pos_output = [random_factory.warehouses[i].pos_x + 1,
                                                   random_factory.warehouses[i].pos_y + 2]
        chosen_input_product = random.choice(list(random_input_products.keys()))
        try:
            chosen_output_product = random.choice(
                [x for x in list(random_output_products.keys()) if x != chosen_input_product])
    ...
    for i in range(amount_of_agvs):
        ...
        random_factory.agvs.append(AGV(start_position=[random_x_position, random_y_position], static=STATIC))
        random_factory.agvs[i].name = f'AGV_{i}'
        random_factory.agvs[i].thread_running = False
    ...
    for x in range(random_factory.length // 4 + 1):
        for y in range(random_factory.width):
            random_factory.factory_grid_layout[4 * x][y] = Path()
    for y in range(random_factory.width // 4 + 1):
        for x in range(random_factory.length):
            random_factory.factory_grid_layout[x][4 * y] = Path()
...
def create_random_factory():
    ...
    grids_x = 8
    grids_y = 5
    random_factory.length = (grids_x + 1) + 3 * grids_x
    random_factory.width = (grids_y + 1) + 3 * grids_y
    ...
    amount_of_machines = 19
    amount_of_warehouses = 1
    amount_of_agvs = 6
    amount_of_products = amount_of_machines + amount_of_warehouses
    STATIC = True
    create_random_products(random_factory, amount_of_products)
    objects = create_random_objects(random_factory, amount_of_machines, amount_of_warehouses, amount_of_agvs, STATIC)
    if objects == False:
        create_random_factory()
    else:
        now = dt.datetime.now().strftime("%Y%m%d_%H-%M-%S")
        print(now)
        filename = f"Random_Path_Factories/{now}_random_factory.pkl"
        # with open(f"Random_Path_Factories/{now}_random_factory.pkl", 'wb') as outp:  # local start
        #     pickle.dump(random_factory, outp)
        with open(f"VRP_Simulation/Random_Path_Factories/{now}_random_factory.pkl",
                  'wb') as outp:  # start from test_file
            pickle.dump(random_factory, outp)
